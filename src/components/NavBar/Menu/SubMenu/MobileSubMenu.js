...
export const MobileSubMenu = ({mens, women, kids, subMenu, setSubMenu}) => {
   ...
   const NavClick = () => {
    ...
    return (
        ...
        {kids ? NavLinks[2].map((link, i) => <NavLink onClick={NavClick} key={i} to={`/products?gender=${link.split(' ')[0].substring(0,3).toLowerCase()}&type=${link.split(' ')[1].toLowerCase()}`}>{link}</NavLink>): null}
           </ul>
        </div>
    )
}